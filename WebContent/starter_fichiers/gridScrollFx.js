...
;( function( window ) {
	...
	function GridItem( el ) {
		this.el = el;
		this.anchor = el.querySelector( 'div' );
	...
	GridItem.prototype.addCurtain = function() {
		...
		this.curtain = document.createElement( 'span' );
	...
	GridItem.prototype.changeAnimationDelay = function( time ) {
		...
		if( this.text ) {
			console.log('text');
			this.text.style.WebkitAnimationDelay = time + 'ms';
			this.text.style.animationDelay = time + 'ms';
		}
	}
